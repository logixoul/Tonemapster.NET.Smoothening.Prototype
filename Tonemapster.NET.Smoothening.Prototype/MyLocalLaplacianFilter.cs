...
namespace Tonemapster.NET.Smoothening.Prototype
{
    ...
    public class MyLocalLaplacianFilter : IFilter
    {
        ...
        public MyLocalLaplacianFilter(int radius = 2, float sigma = 0.05f, int n = 10, int levels = 10, float factor = -1.0f)
        {
            this.Radius = radius;
            this.Sigma = sigma;
            this.N = n;
        ...
        private static float[] Rec(float i, float step, int length)
        {
            ...
            {
                table[x] = MyLocalLaplacianFilter.Rec(x / (float)length, i, step);
            }
            return table;
        }
        /// <summary>
        ...
        private static float[] Rem(float sigma, float factor, float i, int length)
        {
            ...
            for (int x = 0; x < length; x++)
            {
                table[x] = MyLocalLaplacianFilter.Rem(x / (float)length, sigma, factor, i);
            }
            return table;
        }
        #endregion
